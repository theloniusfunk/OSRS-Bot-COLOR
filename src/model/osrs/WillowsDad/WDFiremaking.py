...
class OSRSWDFiremaking(WillowsDadBot):
    def __init__(self):
        bot_title = "WD Firemaking"
        description = "Place this near a bank with logs and it will craft fires near you. Note: AFK doesn't always switch screens."
    ...
    def main_loop(self):
        ...
        while time.time() - self.start_time < self.end_time:
            ...
            try:
                ...
                if len(self.api_m.get_inv_item_indices(ids.logs)) == 0:
                    self.open_bank() #looks for yellow tagged bank
                    #time.sleep(self.random_sleep_length()/2) #does not apply
                    while not self.api_m.get_is_player_idle(): #ADDED delay
    ...
    def setup(self):
        """Sets up loop variables, checks for required items, and checks location.
            ...
            Returns:
                None"""
        super().setup()
        self.withdraw_ids = [ids.MAPLE_LOGS]
        self.withdraw_paths = [self.WILLOWSDAD_IMAGES.joinpath("Maple_logs_bank.png")]
        self.deposit_ids = [ids.ASHES]
        #self.supercompost = 0
